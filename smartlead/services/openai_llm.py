...
class OpenAILLM:
    """Same interface as GeminiLLM: generate_text / generate_json."""

    def __init__(self, settings: Settings) -> None:
        key = (settings.openai_api_key or "").strip()
        if not key:
            raise ValueError(
                "OPENAI_API_KEY or OPENAPI_API_KEY is required when LLM_PROVIDER=openai.",
            )
        self._client = OpenAI(api_key=key)
        self._model = (settings.openai_model or "").strip() or "gpt-4o-mini"
